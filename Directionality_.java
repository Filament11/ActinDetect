...
// re-check if this actually returns null
			return null;
		...
		for (int i=0; i<files.length; i++) {
			...
			if (img!=null){
				...
				//System.out.println(pixels[0][99]);
				//float [][] normal_pix = normalise(pixels, 0, 1, ip.getWidth(), ip.getHeight());
				//System.out.println(normal_pix[0][99]);
				return img;
			}
		}  
		return null;
...
// prints all the path directories of files selected
	    for(int i = 0; i<files.length; i++)
	    {
...
// Run method, aka the main method for image j

	public void run(String arg) {
		
// this is commented out because this requires the files to be open
// before the program runs, replaced with Chooser and openFiles
...
// end of the run method
	...
	*/
	...
	public void computeHistograms() {
		System.out.println("Inside computeHistograms ");
		if (null == imp) return;
...
// lower the variance in data, the higher the sobel filter kernel can be
// 5 by 5 filter is less sensative to noise than 3 by 3
		...
		try
		{
			PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter("/Users/aishik/Desktop/JavaAngles.txt")));
			System.out.println("looping through the for loop");
